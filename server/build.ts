import { basename, dirname, extname, join } from "https://deno.land/std@0.141.0/path/mod.ts";
import { ensureDir } from "https://deno.land/std@0.141.0/fs/ensure_dir.ts";
import { build as esbuild, type Loader, stop } from "https://deno.land/x/esbuild_deno@v0.14.42/native/mod.js";
...
export async function build(serverEntry?: string) {
  ...
  const serverEntryCode = [
    ...
    `import { globToRegExp } from "https://deno.land/std@0.141.0/path/mod.ts";const moduleLoaders = []; globalThis["__ALEPH_MODULE_LOADERS"] = moduleLoaders;`,
    moduleLoaders.length > 0 &&
